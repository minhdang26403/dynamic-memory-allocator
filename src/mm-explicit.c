...
void *realloc(void *oldptr, size_t size)
{
    ...
    if (newsize <= oldsize) {
        ...
        PUT(HDRP(NEXT_BLKP(oldptr)), PACK(oldsize - newsize, 1));
        PUT(FTRP(NEXT_BLKP(oldptr)), PACK(oldsize - newsize, 1));
        free(NEXT_BLKP(oldptr));
        return oldptr;
    } else {
        size_t next_alloc, next_size, csize;
        ...
        if (!next_alloc && (csize = oldsize + next_size) >= newsize) {
            remove_from_free_list(NEXT_BLKP(oldptr));
